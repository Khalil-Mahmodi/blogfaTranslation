jQuery(function ($) {
    ...
    $('#post-box').each(function(){
        if($(this).children().find('img').length){
            var findImg = $(this).find('img:first-of-type').attr('class','img-responsive').clone();
            $(this).find('#post-img').append(findImg);
        }
    });

    var windowURL=window.location.href;windowURL.indexOf("post")>-1&&($(".post-box").addClass('set'));
